//! Minimal, allocation-light logging - same shape as mitos-services' own
//! `logging.rs` (duplicated rather than shared, matching that crate's
//! established reasoning: keeping every MITOS binary independently
//! buildable without a workspace-internal library crate). Writes to
//! `/dev/kmsg` when it exists, falling back to stdout/stderr.
...
fn write_line(level: Level, tag: &str, msg: &str) {
    ...
    let line = format!("mitos-service [{tag:>4}] [{:>8.3}] {msg}\n", uptime_secs());
